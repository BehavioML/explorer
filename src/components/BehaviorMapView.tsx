...
import { layoutBehaviorMapGraph } from '../model-map/behaviorMapLayout';
  ...
  return (
    <section className="behavior-map-view" aria-labelledby="behavior-map-title">
      <div className="behavior-map-toolbar">
        ...
        <button type="button" onClick={expandSelected}>Expand one level</button>
        ...
        <button type="button" onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}>Fit to view</button>
      ...
      <svg
        ...
        onPointerDown={(event) => setDragStart({ x: event.clientX - pan.x, y: event.clientY - pan.y })}
      ...
      >
        <g transform={`translate(${pan.x} ${pan.y}) scale(${zoom})`}>
          {layout.edges.map((edge) => <path className={`behavior-map-edge behavior-map-edge--${edge.kind}`} key={edge.id} d={`M ${edge.sourceX} ${edge.sourceY} C ${(edge.sourceX + edge.targetX) / 2} ${edge.sourceY}, ${(edge.sourceX + edge.targetX) / 2} ${edge.targetY}, ${edge.targetX} ${edge.targetY}`} />)}
          {layout.nodes.map((node) => (
            <g className={`behavior-map-node behavior-map-node--${node.kind} ${node.workflowSubtype ? `behavior-map-node--${node.workflowSubtype}` : ''} ${isSelected(node, selectedEntity) ? 'behavior-map-node--selected' : ''}`} key={node.id} transform={`translate(${node.x} ${node.y})`} onClick={(event) => { event.stopPropagation(); selectNode(node); }} onDoubleClick={(event) => { event.stopPropagation(); toggleNode(node); }}>
              <title>{node.label} — {node.ref}{node.workflowSubtype === 'aggregated' ? ' (aggregate)' : ''}</title>
              {node.shape === 'pill' ? <rect x={-node.width / 2} y={-node.height / 2} width={node.width} height={node.height} rx={22} /> : <circle r={node.radius} />}
              <text className="behavior-map-label" textAnchor="middle" dominantBaseline="middle"><tspan x="0">{node.label}</tspan>{node.kind === 'semantic-area' ? <tspan x="0" dy="16">{node.sizeWeight} workflows</tspan> : null}{node.workflowSubtype === 'aggregated' ? <tspan x="0" dy="16">aggregate</tspan> : null}</text>
